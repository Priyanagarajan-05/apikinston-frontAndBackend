...
namespace KinstonBackend.Controllers
{
    ...
    public class ProfessorsController : ControllerBase
    {
        ...
        // Get pending professors (IsActive == 0)
        [HttpGet("pending")]
        public IActionResult GetPendingProfessors()
        {
            var pendingProfessors = _context.Users
                .Where(u => u.Role == "Professor" && u.IsActive == 0)
                .ToList();

            return Ok(pendingProfessors);
        }

        // Approve a professor (set IsActive to 1)
        [HttpPost("approve/{id}")]
        public IActionResult ApproveProfessor(int id)
        {
            var professor = _context.Users.FirstOrDefault(u => u.Id == id && u.Role == "Professor");
            if (professor == null)
            {
                return NotFound();
            }
